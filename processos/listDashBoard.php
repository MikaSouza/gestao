...
function getGrupoVinculoEmpregaticio() {
	$sql = "SELECT						
				COUNT(u.USUVINCULO) as TOTAL,
				t.TABDESCRICAO
			FROM USUARIOS u
			LEFT JOIN TABELAS t ON t.TABCODIGO = u.USUVINCULO
			WHERE
				u.USUSTATUS = 'S' AND 
				u.USUDATADEMISSAO IS NULL AND
				u.USUVINCULO IS NOT NULL AND u.USUVINCULO <> ''	
			GROUP BY u.USUVINCULO
			ORDER BY TOTAL DESC	";			
	$arrayQueryFilho = array(
						'query' => $sql,
						'parametros' => array()
...
function getGrupoCargos() {
	$sql = "SELECT						
				COUNT(u.TABCARGO) as TOTAL,
				t.TABDESCRICAO
			FROM USUARIOS u
			LEFT JOIN TABELAS t ON t.TABCODIGO = u.TABCARGO
			WHERE
				u.USUSTATUS = 'S' AND 
				u.USUDATADEMISSAO IS NULL AND	
				u.TABCARGO IS NOT NULL AND u.TABCARGO <> ''
			GROUP BY u.TABCARGO
			ORDER BY TOTAL DESC	";			
	$arrayQueryFilho = array(
						'query' => $sql,
...
<html lang="pt-br">
    ...
    <body>
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
						...
						<div class="col-6"> 
							<div class="card">
								<div class="card-body">
									<h4 class="header-title mt-0 mb-4">Vínculo Empregatício</h4>
									<div class="">
										<div id="div_vinculo_empregaticio" class="apex-charts"></div>
									</div>                                        
								</div><!--end card-body-->
							</div><!--end card-->
						</div><!--end col-->
						<div class="col-6"> 
							<div class="card">
								<div class="card-body">
									<h4 class="header-title mt-0 mb-4">Cargos</h4>
									<div class="">
										<div id="div_cargos" class="apex-charts"></div>
									</div>                                        
								</div><!--end card-body-->
							</div><!--end card-->
						</div><!--end col-->   
		...
		<?php
			...
			$global_vinculo_empregaticio = getGrupoVinculoEmpregaticio();
			$vASeriesVE = json_encode($global_vinculo_empregaticio[0]);
			$vALabelsVE = json_encode($global_vinculo_empregaticio[1]);
			
			$global_cargos = getGrupoCargos();
			$vASeriesC = json_encode($global_cargos[0]);
			$vALabelsC = json_encode($global_cargos[1]);
		...
		<script type="text/javascript" DEFER="DEFER">
			...
			var dataObject = {
				...
				width: 480,
				height: 480,
				div_retorno: '#div_vinculo_empregaticio',
				series: <?= $vASeriesVE;?>,
				labels: <?= $vALabelsVE;?>
			}
			chartPie( dataObject );
