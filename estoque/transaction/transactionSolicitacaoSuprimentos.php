...
function listSolicitacaoSuprimentos($_POSTDADOS){
	...
	if(verificarVazio($_POSTDADOS['FILTROS']['vSSOEPOSICAO']) && ($_POSTDADOS['FILTROS']['vSSOEPOSICAO'] != "T")) {
		if (($_POSTDADOS['FILTROS']['vSSOEPOSICAO'] == "AGUARDANDO") || ($_POSTDADOS['FILTROS']['vSSOEPOSICAO'] == "ENTREGUE"))
			$where .=" AND s.SOEPOSICAO = ? AND s.SOESTATUS = 'S' ";
		else if($_POSTDADOS['FILTROS']['vSSOEPOSICAO'] == "D")
			$where .=" AND (s.SOEPOSICAO = ? OR s.SOESTATUS = 'N') ";
	}
