...
if ($vROBJETO['SOESOLICITANTE'] == '') $vROBJETO['SOESOLICITANTE'] = $_SESSION['SI_USUCODIGO'];
?>
<!DOCTYPE html>
<html lang="pt-br">
    <head>
    ...
    <body onload="comboDepartamentos(<?= $vROBJETO['SOESOLICITANTE'];?>, 'vHTABDEPARTAMENTO');">
                                    ...
                                    <form class="form-parsley" action="#" method="post" name="form<?= $vAConfiguracaoTela['MENTITULOFUNC'];?>" id="form<?= $vAConfiguracaoTela['MENTITULOFUNC'];?>">
										...
										<div class="form-group row">
											<div class="col-md-6">
												...
												<select name="vISOESOLICITANTE" id="vISOESOLICITANTE" class="custom-select obrigatorio" title="Solicitante" onchange="comboDepartamentos(this.value);">
													<option value="">  -------------  </option>
													<?php foreach (comboUsuarios() as $usuarios): ?>                                                            
														<option value="<?php echo $usuarios['USUCODIGO']; ?>" <?php if ($vROBJETO['SOESOLICITANTE'] == $usuarios['USUCODIGO']) echo "selected='selected'"; ?>><?php echo $usuarios['USUNOME']; ?></option>
													<?php endforeach; ?>
												</select>
											</div>
											<div class="col-md-6">
												<div id="div_departamento"></div>
											</div>						
										</div>	
										...
										<div class="form-group row">
											<div class="col-md-2">
												<label>Posição</label>
												<select class="form-control obrigatorio" title="Posição" name="vSSOEPOSICAO" id="vSSOEPOSICAO">
													<option value="">-------------</option>
													<option value="AGUARDANDO" <?php if ($vROBJETO['SOEPOSICAO'] == "AGUARDANDO") echo "selected='selected'"; ?>>AGUARDANDO ENTREGA</option>
													<option value="ENTREGUE" <?php if ($vROBJETO['SOEPOSICAO'] == "ENTREGUE") echo "selected='selected'"; ?>>ENTREGUE</option>
													<option value="CANCELADA" <?php if ($vROBJETO['SOEPOSICAO'] == "CANCELADA") echo "selected='selected'"; ?>>CANCELADA</option>
												</select>
											</div>
											<div class="col-md-2">
												<label>ID</label>
												<input type="text" class="form-control" name="vH<?= $vAConfiguracaoTela['MENPREFIXO'];?>" id="vH<?= $vAConfiguracaoTela['MENPREFIXO'];?>CODIGO" title="ID" value="<?= adicionarCaracterLeft($vROBJETO['SOECODIGO'], 6); ?>" readonly/>												
											</div>
										</div>
										<div class="form-group">
											<label class="form-check-label is-invalid" for="invalidCheck3" style="color: red">
