...
// criptografia
	...
	***************************************************/
	...
	function sql_add_valores_between($pSCampoBanco, $pCValorInicio, $pCValorFinal){
		...
		else if(verificarVazio($pCValorInicio) && !verificarVazio($pCValorFinal))
			$vSSql =" and (".$pSCampoBanco." >= '".formatar_valor_monetario_banco($pCValorInicio)."')";
		else if(verificarVazio($pCValorFinal) && !verificarVazio($pCValorInicio))
...
function valorPorExtensoPorcentagem($valor=0) {
    if ((int)$valor > 0) {
        ...
        for ($i=0;$i<count($inteiro);$i++) {
            ...
            if ($valor == "000")$z++; elseif ($z > 0) $z--;
            if (($t==1) && ($z>0) && ($inteiro[0] > 0)) $r .= (($z>1) ? " de " : "").$plural[0];
            if ($r) $rt = $rt . ((($i > 0) && ($i <= $fim) && ($inteiro[0] > 0) && ($z < 1)) ? ( ($i < $fim) ? ", " : " e ") : " ") . $r;
        }
