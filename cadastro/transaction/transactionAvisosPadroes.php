...
if (($_POST["methodPOST"] == "insert")||($_POST["methodPOST"] == "update")) {
    $vIOid = insertUpdateAvisosPadroes($_POST, 'S');
    return;
} else if (($_GET["method"] == "consultar")||($_GET["method"] == "update")) {
    $vROBJETO = fill_AvisosPadroes($_GET['oid'], $vAConfiguracaoTela);
...
function insertUpdateAvisosPadroes($_POSTDADOS, $pSMsg = 'N'){
	$dadosBanco = array(
		'tabela'  => 'AVISOSPADROES',
		'prefixo' => 'AVP',
		'fields'  => $_POSTDADOS,
		'msg'     => $pSMsg,
		'url'     => 'cadAvisosPadroes.php',
		'debug'   => 'N'
		);
...
function fill_AvisosPadroes($pOid){
	$SqlMain = 'SELECT c.*
				 From AVISOSPADROES c
				 Where c.AVPCODIGO = '.$pOid;
