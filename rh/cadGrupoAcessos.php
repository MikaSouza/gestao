									...
									<form class="form-parsley" action="#" method="post" name="form<?= $vAConfiguracaoTela['MENTITULOFUNC'];?>" id="form<?= $vAConfiguracaoTela['MENTITULOFUNC'];?>" onSubmit="return validarForm();">
										...
										<div class="form-group row">
											<div class="col-md-6">
												...
												<select title="Perfil" id="vIGRAPERFIL" class="custom-select obrigatorio" name="vIGRAPERFIL">
													<?php foreach (comboTabelas('USUARIOS - GRUPOS ACESSO') as $tabelas): ?>
														<option value="<?php echo $tabelas['TABCODIGO']?>" <?php if ($vROBJETO['GRAPERFIL'] == $tabelas['TABCODIGO']) echo "selected"; ?>><?= $tabelas['TABDESCRICAO']?></option>
													<?php endforeach; ?>
												</select>
											</div>                                           
										</div>
