...
function excluirRegistroGridCapa(pOID, pSTransaction, pSMethod, pIOIDMENU,pSList){
...
function exibirFormModal(idFilho,div_nome,titulo) {
	if (idFilho != ''){
		...
		if(titulo === "ClientesxHistorico"){
			...
			fillContatos(idFilho, titulo);
		}else if(titulo == "ClientesxEnderecos"){
			fillEndereco(idFilho, titulo);
		}
	}else{
...
function exibirFormModalIncluirAlterarTipoTabela(pAParameters)
{
    ...
    if((pAParameters['pSMethod'] == 'alterar') || (pAParameters['pSMethod'] == 'desativar'))
    {

       	if(!vITABCODIGO)
       	{
		   sweetAlert("Atenção", "O campo "+pAParameters['vSTitulo']+" deve ser selecionado. ", "warning");
            return false;
        }
        else
        {
			if(pAParameters['pSMethod'] == 'alterar') {
				...
				var vIItemSelecionado = $("#"+pAParameters['vSCampo']+" option:selected");
				$("#vHTABCODIGODIALOG").val($("#"+pAParameters['vSCampo']).val());
