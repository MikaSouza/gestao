...
if (($_POST["methodPOST"] == "insert")||($_POST["methodPOST"] == "update")) {
    ...
    $vROBJETO = fill_AgendaGenericaTela($_GET['oid'], $vAConfiguracaoTela);
...
if (isset($_POST['hdn_metodo_search'])) {
	...
	switch ($_POST['hdn_metodo_search']) {
		case 'searchDashBoard':
			search_AgendaGenerica($_POST);
			break;
		case 'searchAXA':
			search_AgendaGenericaSubGrid($_POST['vIATECODIGO']);
			break;
		case 'searchANX':
			getAnexosAgendaGenerica($_POST);
			break;
...
if ($_GET['method'] == 'list') {
    unset($_GET['list']);
    echo json_encode(listAgendaGenericaCalendario($_GET));
}
if ($_GET['method'] == 'delete') {
    echo json_encode(deleteAgendaGenerica($_GET['vIAGECODIGO']));
}
if ($_POST['hdn_metodo_insert'] == 'insertAgendaGenerica') {
	$_POST['vSAGEDATAFINAL'] = $_POST['vSAGEDATAINICIO'];
    echo json_encode(insertUpdateAgendaGenerica($_POST, 'N'));
}

if (isset($_GET['hdn_metodo_fill']) && $_GET['hdn_metodo_fill'] == 'fill_AgendaGenericaCalendario')
	fill_AgendaGenericaCalendario($_GET['AGECODIGO'], $_GET['formatoRetorno']);
...
function comboAgendaGenericaHome($pSTIPO)
{
...
function listAgendaGenericaCalendario($dados)
{
	pre($dados);
    $cod_usuario = filter_var($_SESSION['SI_USUCODIGO'], FILTER_SANITIZE_NUMBER_INT);
...
function listAgendaGenerica($_POSTDADOS){
	$where = '';
...
function fill_AgendaGenerica($codigo)
{
...
function deleteAgendaGenerica($vIAGECODIGO)
{
    return insertUpdateAgendaGenerica(
        array(
...
function fill_AgendaGenericaCalendario($pOid, $formatoRetorno = 'array' ){
	$SqlMain = 	"SELECT
...
function fill_AgendaGenericaTela($pOid){
	$SqlMain = "SELECT
	                a.*, c.CLINOMEFANTASIA AS CLIENTE,
					ctr.CTRNROCONTRATO,
					con.CONNOME
